...
namespace DAL
{
    ...
    public class dalLeaveword
    {
        ...
        {
            string sql = "insert into Leaveword(leaveTitle,leaveContent,userObj,leaveTime,replyContent,replyTime) values(@leaveTitle,@leaveContent,@userObj,@leaveTime,@replyContent,@replyTime)";
            /*构建sql参数*/
            ...
            parm[2].Value = leaveword.userObj; //留言人
            parm[3].Value = leaveword.leaveTime; //留言时间
            parm[4].Value = leaveword.replyContent; //老师回复
            parm[5].Value = leaveword.replyTime; //回复时间
        ...
        public static ENTITY.Leaveword getSomeLeaveword(int leaveWordId)
        {
            ...
            if (DataRead.Read())
            {
                ...
                leaveword.replyTime = Convert.ToDateTime(DataRead["replyTime"].ToString());
            }
            return leaveword;
        }
        ...
        {
            string sql = "update Leaveword set leaveTitle=@leaveTitle,leaveContent=@leaveContent,userObj=@userObj,leaveTime=@leaveTime,replyContent=@replyContent,replyTime=@replyTime where leaveWordId=@leaveWordId";
            /*构建sql参数信息*/
            SqlParameter[] parm = new SqlParameter[] {
             new SqlParameter("@leaveTitle",SqlDbType.VarChar),
             new SqlParameter("@leaveContent",SqlDbType.VarChar),
            ...
            parm[2].Value = leaveword.userObj;
            parm[3].Value = leaveword.leaveTime;
            parm[4].Value = leaveword.replyContent;
            parm[5].Value = leaveword.replyTime;
            parm[6].Value = leaveword.leaveWordId;
            /*执行更新*/
            return (DBHelp.ExecuteNonQuery(sql, parm) > 0) ? true : false;
        }
